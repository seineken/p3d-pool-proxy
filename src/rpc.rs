...
impl PoolContex {
    ...
    pub(crate) async fn get_mining_params(&self, pool_id: String) -> anyhow::Result<()> {
        let response: JsonValue = self
            .client
            .request(
                "poscan_getMiningParams",
                rpc_params![serde_json::json!(pool_id)],
            )
            .await?;
        ...
        match (
            ...
            _ => {
                println!("🟥 Ask_mining_params error: Incorrect response from pool node.");
            }
        }
        Ok(())
    }

    pub(crate) async fn push_to_pool(&self, proposal: MiningProposal) -> anyhow::Result<JsonValue> {
